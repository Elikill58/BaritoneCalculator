...
package eli.baritone.pathing.path;

import static eli.baritone.api.pathing.movement.MovementStatus.FAILED;
import static eli.baritone.api.pathing.movement.MovementStatus.SUCCESS;
import static eli.baritone.api.pathing.movement.MovementStatus.UNREACHABLE;
...
import eli.baritone.api.pathing.path.IPathExecutor;
...
import eli.baritone.pathing.movement.movements.MovementAscend;
import eli.baritone.pathing.movement.movements.MovementDescend;
...
public class PathExecutor implements IPathExecutor, Helper {
    ...
    public boolean onTick(TickEvent e) {
        ...
        BetterBlockPos whereAmI = ctx.playerFeetBlockPos();
        ...
        if (movementStatus == SUCCESS) {
            ...
            sprintNextTick = shouldSprintNextTick(e);
            if (!sprintNextTick) {
    ...
    private boolean shouldPause() {
        ...
        if (!MovementHelper.canWalkOn(ctx, ctx.playerFeetBlockPos().down())) {
            // we're in some kind of sketchy situation, maybe parkouring
            return false;
        }
        if (!MovementHelper.canWalkThrough(ctx, ctx.playerFeetBlockPos()) || !MovementHelper.canWalkThrough(ctx, ctx.playerFeet().up())) {
            // suffocating?
            return false;
        }
    ...
    public boolean snipsnapifpossible() {
        if (!ctx.onGround() && !ctx.world().getBlockAt(ctx.playerFeetLocation()).getType().name().contains("WATER")) {
        ...
        int index = path.positions().indexOf(ctx.playerFeetBlockPos());
        if (index == -1) {
            return false;
        }
    ...
    private boolean shouldSprintNextTick(TickEvent e) {
        ...
        if (current instanceof MovementFall) {
            ...
            if (data != null) {
                BetterBlockPos fallDest = new BetterBlockPos(data.b());
    ...
    private static boolean skipNow(PlayerContext ctx, IMovement current) {
        double offTarget = Math.abs(current.getDirection().getX() * (current.getSrc().z + 0.5D - ctx.player().locZ())) + Math.abs(current.getDirection().getZ() * (current.getSrc().x + 0.5D - ctx.player().locX()));
        if (offTarget > 0.1) {
            return false;
        }
        // we are centered
        ...
        // wait 0.3
        double flatDist = Math.abs(current.getDirection().getX() * (headBonk.getX() + 0.5D - ctx.player().locX())) + Math.abs(current.getDirection().getZ() * (headBonk.getZ() + 0.5 - ctx.player().locZ()));
        return flatDist > 0.8;
    }

    private static boolean sprintableAscend(PlayerContext ctx, MovementTraverse current, MovementAscend next, IMovement nextnext) {
        ...
        if (MovementHelper.avoidWalkingInto(ctx.world().getBlockAt(current.getSrc().up(3).toBukkitLocation(ctx.getWorld())))) {
            return false;
        }
        return !MovementHelper.avoidWalkingInto(ctx.world().getBlockAt(current.getDest().up(2).toBukkitLocation(ctx.getWorld()))); // codacy smh my head
    }
    ...
    public String toString() {
    	return "PathExecutor[pathPosition=" + this.pathPosition + ",nbBlockBreak=" + this.toBreak().size() + ",nbBlockPlace=" + this.toPlace().size() + ",nbWalkInto=" + this.toWalkInto().size() + "]";
    }
}
