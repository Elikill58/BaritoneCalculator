...
import baritone.api.nms.ChunkPos;
...
public interface IWorldScanner {
     ...
     */
    List<BlockPos> scanChunkRadius(PlayerContext ctx, BlockOptionalMetaLookup filter, int max, int yLevelThreshold, int maxSearchRadius);
     ...
     * @return The matching block positions
     */
    List<BlockPos> scanChunk(PlayerContext ctx, BlockOptionalMetaLookup filter, ChunkPos pos, int max, int yLevelThreshold);
     ...
     * @return The amount of chunks successfully queued for repacking
     */
    int repack(PlayerContext ctx, int range);
}
